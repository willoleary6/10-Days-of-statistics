...
float geometric(float probabilty, float iteration, float x){
    return pow(probabilty,x) * pow((1-probabilty),(iteration-x));
}
int main() {
    ...
    float results = geometric(propability, iteration, 1);
